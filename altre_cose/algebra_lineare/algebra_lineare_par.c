...
unsigned long add_j_to_k(unsigned long M [][NUM_BLOCCHI], unsigned long k, 
  ...
  //#pragma omp parallel for schedule(dynamic, n_blocchi/2)
...
void get_wt_k(unsigned long M[][NUM_BLOCCHI], unsigned long n_blocchi,
  ...
  //#pragma omp parallel for schedule(dynamic, (n_blocchi * N_BITS)/2)
  for(i = i; i < (n_blocchi * N_BITS); ++i)
    if(get_k_i(M, k, i))
      wt->n_bit++;
...
int main() {
  ...
  int chunck = K/n_threads;

  double t1 = omp_get_wtime();
#pragma omp parallel for schedule(dynamic, K/4)
  for(int i = 0; i < K; ++i)
